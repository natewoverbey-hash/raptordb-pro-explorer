...
const useCases = [
  { id: "service-delivery", icon: <BarChartIcon size={32} />,
    ...
    after: { title: "With RaptorDB Pro",
      ...
      narrative: "Column-store + parallel processing delivers up to 27x faster analytics. Intraday Analytics (Pro-exclusive, powered by Data Snapshots) tracks KPIs as they evolve throughout the day — not just daily summaries — so you can see surge patterns and reallocate staff before queues build. Unlimited PA drilldowns let you slice incidents by any dimension combination with no depth limits, built on Change Data Capture for efficient, scalable analytics." },
    outcomes: [
      { metric: "25x", label: "faster SQL response time (MTS)", type: "proof" },
      { metric: "80%", label: "reduction in UI response time (MTS)", type: "proof" },
      { metric: "45%", label: "faster processing (Amadeus)", type: "proof" },
    ],
    proExclusive: ["Intraday Analytics (Data Snapshots)", "Unlimited PA Drilldowns", "Column-Store Index"],
  },
  { id: "bi-analytics", icon: <TrendingUpIcon size={32} />,
    ...
    after: { title: "With RaptorDB Pro",
      ...
      narrative: "SQL API with JDBC/ODBC drivers pushes SQL directly to RaptorDB Pro — data stays in ServiceNow while external tools query and visualize it securely (zero-copy access). Read-only with full ACL support means governance stays intact. Cross-product analytics in a single HTAP database — ITSM + HRSD + CSM + CMDB in one query, one source of truth. Archive tables are also queryable via SQL API, so historical data is just as accessible." },
    outcomes: [
  ...
  { id: "ai-agentic", icon: <BotIcon size={32} />,
    ...
    after: { title: "With RaptorDB Pro",
      ...
      narrative: "HTAP architecture handles both agent transactional actions AND the analytical reasoning agents need — simultaneously. Parallel processing supports hundreds of concurrent AI agent sessions without degradation. Archived data is available to AI Search and LLMs — archive tables look and feel like primary tables, so agents can reason on historical context at speed. RaptorDB Pro is the database foundation in ServiceNow's AI Platform — giving AI agents enterprise context at speed." },
    outcomes: [
      { metric: "3x", label: "throughput for concurrent workloads", type: "verified" },
      { metric: "59%", label: "reduction in compute time (ProAssurance)", type: "proof" },
      { metric: "2-3x", label: "faster workflows (Wells Fargo)", type: "proof" },
    ],
  ...
  { id: "vuln-response", icon: <ShieldIcon size={32} />,
    ...
    tagline: "When your Vulnerable Items table holds tens of millions of records and every scan cycle adds more",
    persona: "CISO / VP Security Operations",
    before: { title: "Today on RaptorDB Standard",
      items: [
        { label: "VI Table Size", value: "Tens of millions", icon: <DatabaseIcon size={16} /> },
        { label: "Prioritization Speed", value: "Hours/Days", icon: <ClockIcon size={16} /> },
        { label: "Audit Readiness", value: "Manual exports", icon: <LockIcon size={16} /> },
      ],
      narrative: "Every scanner (Tenable, Qualys, Rapid7, Microsoft Defender) ingests every CVE against every asset on every scan cycle. For large health systems with millions of assets, this creates tens of millions of Vulnerable Items records turning over constantly. Only 2-5% pose real exploitable risk, but the database must process 100% to surface that signal. ServiceNow publishes a dedicated VR Performance Best Practices Guide precisely because high-volume VR environments break standard configurations." },
    after: { title: "With RaptorDB Pro",
      items: [
        { label: "VI Table Speed", value: "Seconds", icon: <ZapIcon size={16} /> },
...
const discoveryQuestions = [
  ...
  { id: "q8", question: "How large is your Vulnerability Response footprint — and how do you prioritize remediation?",
    options: [
      { label: "Small VR footprint, manageable volume", value: 0, tags: [] },
      { label: "Significant volume — prioritization takes hours", value: 1, tags: ["vuln-response"] },
      { label: "Tens of millions of Vulnerable Items — it's a major bottleneck", value: 2, tags: ["vuln-response", "compliance"] },
    ]},
];
